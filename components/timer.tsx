...
export default function Timer({ type }: { type: string }) {
  ...
  useEffect(() => {
    const countDownDate = new Date('August 31, 2023 15:37:25').getTime()
  ...
  return type === 'vertical' ? (
    <div id="timer" className="mt-5">
      <div className="bg-skin-secondary inline-block justify-center rounded-xl p-2 align-middle">
        <div className="flex">
          ...
          <span className="mr-4 flex flex-col p-2">
            <span>{timer.hr}</span>
            <span className="text-xs text-green-500">Hrs</span>
          </span>
          <span className="mr-4 flex flex-col p-2">
            <span>{timer.min}</span>
            <span className="text-xs text-green-500">mins</span>
          </span>
          <span className="mr-4 flex flex-col p-2">
            <span>{timer.sec}</span>
            <span className="text-xs text-green-500">Sec</span>
          </span>
        </div>
    ...
    <div id="timer" className="mt-4">
      <div className="bg-skin-secondary inline justify-center space-y-6 rounded-xl align-middle">
        <div className="bg-skin-secondary mt-5 inline-block justify-center rounded-xl  align-middle ring ring-green-500">
          <div className="flex">
            ...
            <span className="md:mr1 mr-2 flex flex-col p-2">
              <span>{timer.hr}</span>
              <span className="text-xs text-green-300">Hrs</span>
            </span>
            <span className="md:mr1 mr-2 flex flex-col p-2">
              <span>{timer.min}</span>
              <span className="text-xs text-green-300">mins</span>
            </span>
            <span className="mr-4 flex flex-col p-2">
              <span>{timer.sec}</span>
              <span className="text-xs text-green-300">Sec</span>
            </span>
