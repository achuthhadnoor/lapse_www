...
export default function Header() {
  ...
  const NavMenu = () => (
    <nav className="flex flex-col items-end gap-4 rounded align-middle md:flex-row">
      <Link href={'/'}>
        <span className="px-2 py-1">Home</span>
      </Link>
      <Link href={'#features'}>
        <span className="px-2 py-1">Features</span>
      </Link>
      {/* <Link href={'/'}>
      ...
      <Link href={'#feedback'}>
        <span className="px-2 py-1">Feedback</span>
      </Link>
    </nav>
  )
  return (
    <header className="mx-auto flex max-w-6xl items-center justify-between gap-4 py-4 px-2 align-middle lg:px-0">
      <span className="hidden bg-gradient-to-r from-green-400 to-green-700/40 bg-clip-text text-3xl font-bold text-transparent md:inline">
        Lapse
      </span>
      ...
      {open && (
        <div className="absolute left-0 top-0 w-full p-4">
          <div className="flex flex-col items-end gap-4 rounded-lg bg-black/40 p-4 backdrop-blur-2xl">
            <button
              onClick={() => {
                setOpen(false)
              }}
